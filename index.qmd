...
## Analysis: Answering the Challenge Questions
...
print(f"Intercept (β₀): {model_stresssurvey.params['const']:.4f}")
print(f"StressSurvey coefficient (β₁): {model_stresssurvey.params['StressSurvey']:.4f}")
print(f"\nR-squared: {model_stresssurvey.rsquared:.4f}")
```
...
The bivariate regression of Anxiety on StressSurvey yields the following estimated coefficients (see output above for exact values):

- **Intercept (β₀):** The estimated intercept differs from the true value of 0
- **StressSurvey coefficient (β₁):** The estimated coefficient differs from the true Stress coefficient of 1
- **R-squared:** See output above

**Comparison to True Relationship:**

The true relationship is: $Anxiety = Stress + 0.1 \times Time$

When we regress Anxiety on StressSurvey alone, we're not capturing the true relationship because:

1. **StressSurvey is a proxy for Stress**, not Stress itself. The relationship between StressSurvey and actual Stress is non-linear (as shown in @fig-stress-proxy).

2. **The true coefficient on Stress is 1**, meaning a one-unit increase in actual Stress leads to a one-unit increase in Anxiety. However, when we use StressSurvey as the predictor, the estimated coefficient will be different from 1. This difference occurs because StressSurvey has a non-linear relationship with actual Stress, and the regression is trying to fit a linear relationship to a non-linear pattern.

3. **The model omits Time**, which has a true coefficient of 0.1. This omission means the regression is trying to capture both the Stress effect and the Time effect through the StressSurvey coefficient alone, further distorting the estimated coefficient.

4. **The intercept is not zero** as in the true relationship, because the non-linear relationship between StressSurvey and Stress, combined with the omitted Time variable, forces the regression line to adjust to minimize the sum of squared errors.

This demonstrates a key problem: even when we have a "good" proxy variable (StressSurvey is monotonically related to Stress), the non-linear relationship between the proxy and the true variable, combined with omitted variables, can lead to estimated coefficients that are quite different from the true relationship.
